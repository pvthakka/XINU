...
userpid = create(main, INITSTK, INITPRIO, INITNAME, INITARGS);
	...
	//enable_paging(proctab[NULLPROC].pd->frame_num*NBPG);
	//kprintf("\nthis is %d",proctab[NULLPROC].pd->frame_num);
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i = 0; i < 4; i++)
	{
		...
		//kprintf("\nUsing frame %d as shared page table number %d",frame_num,i+1);
		//kprintf("\npd[%d].pd_base = %d",i, frame_num);
		ptr1++;
		frame_num++;
	}
	pptr->pdbr = NBPG*(FRAME0+4);
	//kprintf("\nUsing frame %d as shared page directory",(FRAME0+4));
	pptr->pd = &frm_tab[4];

	/*kprintf("\nreading page tables:");
	
	ptr = (pt_t *)(NBPG*FRAME0);
	for(outer = 0; outer < 4; outer++)
	{
		for(i = 0; i < NFRAMES; i++)
		{
